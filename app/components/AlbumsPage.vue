<template>
  <Page>
    <ActionBar>
      <Label text="Топ альбомов keyseee" />
        ...
        <template v-if="data.length">
          <template v-for="index in top">
            ...
            />
              ...
              />
              <label
                :text="data[index - 1]['artist']['name']"
                col="1"
                row="1"
                class="album-artist"
              />
              <label
                :text="data[index - 1]['playcount']"
                col="2"
                row="0"
                rowSpan="2"
...
export default {
  mounted() {
    Http.getJSON(
      `https://ws.audioscrobbler.com/2.0/?method=user.gettopalbums&user=keyseee&api_key=5546e1e3c0620b921ebe3b90ad4e8214&format=json`
    )
      .then((result) => {
        this.data = result["topalbums"]["album"];
      })
      .catch((error) => {
